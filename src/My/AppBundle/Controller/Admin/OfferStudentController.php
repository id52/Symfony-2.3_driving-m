...
use My\AppBundle\Entity\OfferPriceStudent;
use My\AppBundle\Form\Type\ImageFormType;
use Symfony\Component\Form\FormError;
use Symfony\Component\HttpFoundation\Request;

class OfferStudentController extends AbstractEntityController
{
    protected $perms = array('ROLE_MOD_CONTENT');
    protected $tmplItem = 'OfferStudent/item.html.twig';
    protected $tmplList = 'OfferStudent/list.html.twig';
    ...
    public function itemAction(Request $request)
    {
        ...
        /** @var $entity \My\AppBundle\Entity\OfferStudent */
        ...
        if ($request->isMethod('post')) {
            ...
            if ($form->isValid()) {
                ...
                if ($entity->getImage()) {
                    $entity->getImage()->setOfferStudent(null);
                }
                $image_id = intval($form->get('image_id')->getData());
                $image = $this->em->getRepository('AppBundle:Image')->find($image_id);
                if ($image) {
                    $image->setOfferStudent($entity);
                }

                $this->em->flush();

                $this->em->getRepository('AppBundle:OfferPriceStudent')->createQueryBuilder('op')
                    ->delete()
                    ->where('op.offer_student = :offer')->setParameter('offer', $entity)
                    ->getQuery()->execute();
                ...
                foreach ($regions as $region) { /** @var $region \My\AppBundle\Entity\Region */
                    ...
                    foreach ($categories as $category) { /** @var $category \My\AppBundle\Entity\Category */
                        ...
                        if ($category->getWithAt()) {
                            $price = new OfferPriceStudent();
                            $price->setOfferStudent($entity);
        ...
        foreach ($offer_prices as $offer_price) { /** @var $offer_price \My\AppBundle\Entity\OfferPriceStudent */
